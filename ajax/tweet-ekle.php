...
include ("../inc/head.php");
        ...
        if($_POST)
        {
            ...
            {
                echo json_encode(array('success' => 3, 'icon' => 'info','adi' =>'Lütfen boş alanları doldurun.'));
            }
            else
            {
                $tweet = new tweet();
                ...
                $tweet ->_deleted = 0;
                $sonuc = $tweet->tweetEkle();
                if($sonuc)
                    echo json_encode(array('success' => 1, 'icon' => 'success','adi' =>'Tweet ekleme işlemi başarıyla gerçekleşti.'));
                else
                    echo json_encode(array('success' => 2,'icon' => 'error','adi' =>'Tweet eklerken bir hata ile karşılaşıldı.'));
