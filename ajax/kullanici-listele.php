...
$stmt = db::$conn->prepare("SELECT COUNT(*) AS allcount FROM tb_kullanici WHERE _deleted =0");
$stmt->execute();
$records = $stmt->fetch();
$totalRecords = $records['allcount'];

## Total number of records with filtering
$stmt = db::$conn->prepare("SELECT COUNT(*) AS allcount FROM tb_kullanici WHERE _deleted =0 ".$searchQuery);
$stmt->execute($searchArray);
$records = $stmt->fetch();
$totalRecordwithFilter = $records['allcount'];

## Fetch records
$stmt = db::$conn->prepare("SELECT * FROM tb_kullanici WHERE _deleted =0 ".$searchQuery." ORDER BY ".$columnName." ".$columnSortOrder." LIMIT :limit,:offset");
